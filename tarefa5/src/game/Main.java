...
public class Main {
    ...
    private static void exibirMenuPosCombate(boolean temLoot) {
        ...
        System.out.println("[3] Testar Habilidade Especial");
    ...
    // Simulação básica de interação com loot - CORRIGIDO
    private static void interagirComLoot(Heroi heroi, Monstro monstroDerrotado) {
        ...
        if (temLoot) {
            ...
            // Adicionar uma arma com nível mínimo para demonstrar a exceção
            Espada espadaEpica = new Espada(20, 5, "Espada Épica"); // Nível mínimo 5
            System.out.println("- " + espadaEpica.getNome() + " (Dano: " + espadaEpica.getDano() + ", Nível Mínimo: " + espadaEpica.getminNivel() + ")");
    ...
    private static boolean processarMenuPosCombate(Heroi heroi, Monstro monstroDerrotado) {
        while (true) {
            ...
            int opcao = InputManager.lerInteiro("Escolha uma opção (1-4): ", 1, 4);
            ...
            switch (opcao) {
                ...
                case 2:
                    ...
                    heroi.status();
                    System.out.println("Experiência: " + heroi.getExperiencia() + "/" + heroi.getExperienciaParaProximoNivel());
                    InputManager.esperarEnter("Pressione ENTER para continuar...");
                    return true; // Continuar jogo
                case 3:
                    // Testar habilidade especial (sem alvo, apenas para demonstração)
                    try {
    ...
    // Exibir informações dos heróis
    private static void exibirInfosHerois() {
        Guerreiro guerreiroExemplo = new Guerreiro("Aragorn", 15, 100, null, 1, 0, 100, 0);
        Paladino paladinoExemplo = new Paladino("Uther", 12, 110, null, 1, 0, 100, 0);
        Arqueiro arqueiroExemplo = new Arqueiro("Legolas", 10, 90, null, 1, 0, 100, 0);
    ...
    // Exibir informações dos monstros
    ...
    // Criar herói aleatório - CORREÇÃO: Iniciar com nível 1 e experiência 0
    private static Heroi criarHeroiAleatorio() {
        int tipoHeroi = (int) (Math.random() * 3);
        switch (tipoHeroi) {
            case 0: 
                return new Guerreiro("Aragorn", 15, 100, null, 1, 0, 100, 0);
            case 1: 
                return new Paladino("Uther", 12, 110, null, 1, 0, 100, 0);
            case 2: 
                return new Arqueiro("Legolas", 10, 90, null, 1, 0, 100, 0);
            default: 
                return new Guerreiro("Aragorn", 15, 100, null, 1, 0, 100, 0);
        }
    }

    // Iniciar o jogo - CORREÇÃO COMPLETA COM SISTEMA DE EXPERIÊNCIA FUNCIONAL
    private static void iniciarJogo() {
        ...
        for (int i = 0; i < fases.size(); i++) {
                ...
                for (int j = 0; j < monstros.size(); j++) {
                    ...
                    while (heroi.estaVivo() && monstro.estaVivo()) {
                        ...
                        if (!monstro.estaVivo()) {
                            ...
                            // ✅ CORREÇÃO CRÍTICA: Adicionar experiência ao herói ANTES do menu pós-combate
                            int xpGanha = monstro.getXpConcedido();
                            System.out.println("💡 " + heroi.getNome() + " ganhou " + xpGanha + " de experiência!");
                            heroi.ganharExperiencia(xpGanha);
                        ...
                        if (!heroi.estaVivo()) {
                            System.out.println("\n💀 " + heroi.getNome() + " foi derrotado por " + monstro.getNome() + "! 💀");
                            break;
                        }
                        
                        System.out.println("----- FIM DO TURNO -----");
                        System.out.println(heroi.getNome() + ": " + heroi.getVida() + " HP");
                        System.out.println(monstro.getNome() + ": " + monstro.getVida() + " HP");
                    }

                    if (!heroi.estaVivo()) {
                        System.out.println("\n💀 GAME OVER! " + heroi.getNome() + " foi derrotado! 💀");
                        System.out.println("Nível final: " + heroi.getNivel());
                        System.out.println("Experiência final: " + heroi.getExperiencia() + "/" + heroi.getExperienciaParaProximoNivel());
            ...
            if (i < fases.size() - 1) {
                System.out.println("\n=== FASE " + (i + 1) + " COMPLETADA ===");
                System.out.println("Nível atual: " + heroi.getNivel());
                System.out.println("Experiência: " + heroi.getExperiencia() + "/" + heroi.getExperienciaParaProximoNivel());
        ...
        System.out.println("Nível final: " + heroi.getNivel());
        System.out.println("Experiência total: " + heroi.getExperiencia());
        heroi.status();
        InputManager.esperarEnter("Pressione ENTER para voltar ao menu principal...");
    }
